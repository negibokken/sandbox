...
unordered_map<char, int> createShift(string str) {
  unordered_map<char, int> mp;
  for (int i = 0; i < str.size(); i++) {
    mp[str[i]] = str.size() - i - 1;
  }
  return mp;
}

int bm(string str, string sub) {
  unordered_map<char, int> table = createShift(sub);
  ...
  for (int i = 0, j = 0; i < str.size(); i++) {
    ...
    if (j == sub.size()) return (i + 1) - sub.size();
  }
  return -1;
}
