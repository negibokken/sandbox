...
int maximumEarnings(int initialInvestment, int monthlyContribution,
...
int main(void) {
  ...
  cin >> initialInvestment >> monthlyContribution;
  int m;
  cin >> m;
  vector<string> stockPrices(m);
  REP(i, m) cin >> stockPrices[i];
  REP(i, m) {
    for (int j = 0; j < stockPrices[i].size(); j++) {
      if (stockPrices[i][j] == '-') stockPrices[i][j] = ' ';
    }
  }

  cout << maximumEarnings(initialInvestment, monthlyContribution, stockPrices)
       << endl;
