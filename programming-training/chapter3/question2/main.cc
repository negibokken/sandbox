...
class Val {
 public:
  int val;
  int min;
  Val(int val, int min) : val(val), min(min) {}
};
...
class MinStack {
 ...
 public:
  MinStack(int n) : n(n) { arr.resize(n, {0, 0}); }
  void push(int val) {
    if (size == n) return;
    int mi = (size > 0) ? arr[idx - 1].min : val;
    arr[idx++] = Val(val, std::min(mi, val));
    size++;
  }
  void pop() {
    if (size == 0) return;
