...
TreeNode* rec(int l, int r, vector<int>& nums) {
  ...
  node->left = rec(l, mid, nums);
  node->right = rec(mid + 1, r, nums);
  return node;
}
...
vector<string> printDepthFirst(TreeNode* root) {
  vector<string> ans;
  queue<TreeNode*> q;
  q.push(root);
  while (!q.empty()) {
...
int main(void) {
  ...
  vector<string> ansString = printDepthFirst(ans);
  cout << "[";
