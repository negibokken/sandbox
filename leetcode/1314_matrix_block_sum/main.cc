...
vector<vector<int>> matrixBlockSum(vector<vector<int>>& mat, int K) {
  auto red = [mat](int m, int n, int K) {
    int sum = 0;
    for (int i = max(m - K, 0); i < min(m + K + 1, (int)mat.size()); i++) {
      for (int j = max(n - K, 0); j < min(n + K + 1, (int)mat[0].size()); j++) {
        sum += mat[i][j];
      }
    }
    return sum;
...
int main(void) {
  ...
  vector<vector<int>> ans = matrixBlockSum(arr, K);
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      if (j) cout << " ";
      cout << ans[i][j];
