...
int distanceBetweenBusStops(vector<int>& distance, int start, int destination) {
  int all = 0;
  for (int i = 0; i < distance.size(); i++) all += distance[i];

  int ans = 0;
  int small = start > destination ? destination : start;
  int large = start > destination ? start : destination;
  for (int i = small; i < large; i++) ans += distance[i];
...
int main(void) {
  ...
  cout << distanceBetweenBusStops(arr, start, dest) << endl;
