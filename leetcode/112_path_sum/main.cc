...
bool helper(TreeNode* root, int targetSum, int total) {
  int current = total + root->val;
  if (!root->left && !root->right) {
    return targetSum == current;
  }
  bool f = false;
  if (root->left) f |= helper(root->left, targetSum, current);
  if (root->right) f |= helper(root->right, targetSum, current);
  return f;
}
...
int main(void) {
  ...
  std::cout << (hasPathSum(root, targetSum) ? "true" : "false") << std::endl;
