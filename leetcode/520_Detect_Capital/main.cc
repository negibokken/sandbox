...
bool detectCapitalUse(string word) {
  bool hasCapital = false, hasLower = false, hasMultiCapital = false;
  for (int i = 0; i < word.size(); i++) {
    char c = word[i];
    if (hasCapital && 'A' <= c && c <= 'Z') hasMultiCapital = true;
    if (!hasCapital && 'A' <= c && c <= 'Z') hasCapital = true;
    if (!hasLower && 'a' <= c && c <= 'z') hasLower = true;
    if (hasCapital && hasLower && 'A' <= c && c <= 'Z') return false;
    if (hasMultiCapital && 'a' <= c && c <= 'z') return false;
  }
  return true;
}
int main(void) {
