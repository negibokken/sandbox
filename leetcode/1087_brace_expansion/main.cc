...
vector<vector<char>> preprocess(string s) {
...
void rec(vector<vector<char>>& cand, vector<string>& ans, string s, int idx) {
  if (idx >= cand.size()) {
    ans.push_back(s);
    return;
  }
  for (auto c : cand[idx]) {
    rec(cand, ans, s + c, idx + 1);
  }
}

vector<string> expand(string s) {
  vector<vector<char>> cand = preprocess(s);
  vector<string> ans;
  rec(cand, ans, "", 0);
  return ans;
}
