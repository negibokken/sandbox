...
TreeNode* buildBinaryTree(int l, int r, vector<int>& arr) {
  if (l >= r) return nullptr;
  int mid = (l + r) / 2;
  TreeNode* root = new TreeNode(mid);
  root->left = buildBinaryTree(l, mid, &arr);
  root->right = buildBinaryTree(mid, r, &arr);
  return root;
}
...
TreeNode* balanceBST(TreeNode* root) {
  vector<int> arr;
  dsf(root, arr);
  return buildBinaryTree(0, arr.size(), arr);
}
