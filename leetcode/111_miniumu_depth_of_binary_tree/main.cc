...
int minDepthHelper(TreeNode *root, int cnt) {
  if (root->left == nullptr && root->right == nullptr) {
    return cnt;
  }
  int left = INT_MAX, right = INT_MAX;
  if (root->left != nullptr) left = minDepthHelper(root->left, cnt + 1);
  if (root->right != nullptr) right = minDepthHelper(root->right, cnt + 1);
...
int minDepth(TreeNode *root) {
  ...
  int cnt = minDepthHelper(root, 1);
  return cnt;
}

TreeNode *rec(vector<int> v, int index) {
  if (v[index] == -1 || index >= v.size()) {
    return nullptr;
  }
  TreeNode *node = new TreeNode(v[index]);
  node->left = rec(v, index + 1);
  node->right = rec(v, index + 2);
  return node;
}
...
int main(void) {
  ...
  // 0: root value
  // 0+1: left node value
  // 0+2: right node value
  TreeNode *root = rec(v, 0);
