...
vector<vector<int>> candyCrush(vector<vector<int>>& board) { return board; }
...
int main(void) {
  ...
  MatrixParser* parser = new MatrixParser(s);
  vector<vector<int>> board = parser->parse();

  vector<vector<int>> ans = candyCrush(board);
  print(ans);
